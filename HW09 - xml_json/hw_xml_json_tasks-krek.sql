...
FROM OPENROWSET
(BULK 'C:\Users\79262\OneDrive\Документы\otus-mssql-krek\HW09 - xml_json\StockItems-188-1fb5df.xml', 
 SINGLE_CLOB)
as data 
...
from (
select	StockItemID, StockItemName, CustomFields, tags.[key], tags.value
from Warehouse.StockItems
...
String_Agg(cast(a as nvarchar(max)),',') tags
from (
select StockItemID, StockItemName, CustomFields, tags.[key], tags.value as a
from Warehouse.StockItems
Cross Apply OpenJson(CustomFields, '$.Tags') tags
) tt
